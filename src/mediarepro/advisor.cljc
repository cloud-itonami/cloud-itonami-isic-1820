(ns mediarepro.advisor
  "MediaReproductionAdvisor -- the *contained intelligence node* for
  the media-reproduction plant-operations coordination actor.

  It normalizes production-batch patches (content-type/disc-
  thickness-mm/quantity/defect-rate), drafts a duplication-line-
  equipment maintenance scheduling proposal against a piece of
  equipment, drafts a safety-concern flag, and drafts an outbound
  reproduced-media product shipment coordination proposal against a
  ...
  a committed record and NEVER a real duplication-line-equipment
  actuation, freight dispatch, or copyright reproduction-license/
  clearance authorization -- see README `What this actor does NOT
  do`. Every output is censored downstream by `mediarepro.governor`
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct duplication-
  ...
  `mediarepro.governor` HARD-holds any request that doesn't, so a
  mis-wired caller can never reach a commit path even if this advisor
  were compromised."
  (:require #?(:clj  [clojure.edn :as edn]
               :cljs [cljs.reader :as edn])
            [clojure.string :as str]
            [mediarepro.registry :as registry]
            [mediarepro.store :as store]
            [langchain.model :as model]))

(defn- log-production-batch
  "Production-batch intake upsert -- the advisor only normalizes/
  validates the patch; it does not invent the batch's content-type,
  disc-thickness-mm, quantity, defect-rate or verification status.
...
(defn- schedule-maintenance
  "Draft a duplication-line-equipment maintenance-window scheduling
  ...
  `mediarepro.governor` NEVER trusts this report -- it independently
...
(defn- flag-safety-concern
  "Draft an equipment-safety/quality-defect concern. ALWAYS `:stake
  ...
  `mediarepro.phase`: no phase ever adds this op to a phase's `:auto`
  set; `mediarepro.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound reproduced-media product shipment coordination
  ...
  `mediarepro.governor` NEVER trusts it: it independently recomputes
...
(def ^:private system-prompt
  (str "あなたは記録媒体複製(CD・DVD・Blu-ray・ソフトウェアメディアの"
       "マスターからの複製)工場"
       "プラント運用コーディネーターの助言者です。"
       ...
       "複製ライン等の設備の直接操作(actuate)を絶対に提案してはいけません"
       "(この actor は提案のみを行い、実行は一切行いません)。"
       "著作権のある記録内容の複製許諾/ライセンスを自己発行する提案をしてはいけません"
       "(この actor は権利者・ライセンス団体の代わりに複製許諾を行う権限を持たない)。"
       "出荷数量を偽って報告してはいけません。"))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `mediarepro.governor`
