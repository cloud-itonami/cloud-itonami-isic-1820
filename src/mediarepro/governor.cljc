(ns mediarepro.governor
  "Media Reproduction Plant Operations Governor -- the independent
  compliance layer that earns the MediaReproductionAdvisor the right
  ...
  draft-schedule) duplication-line equipment, whether a proposal
  secretly tries to self-issue a copyright reproduction-license/
  clearance authorization (an authority this actor never holds),
  ...
  stops being a coordination proposal and becomes direct duplication-
  ...
  *reject* a proposal and fall back to HOLD.
  ...
  `:media-reproduction-plant-operations-governor` (see
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       hallucinated `:duplication-line/
                                       actuate` or `:replicator/run`) is
                                       the 'direct duplication-line-
    ...
    5. Copyright-licensing-
       authority blocked           -- ANY proposal (any op) whose own
                                       ...
                                       `:issue-reproduction-license?
                                       true` is attempting to self-
                                       issue a copyright reproduction-
                                       license/clearance authorization
                                       -- an authority exclusively
                                       reserved to the rights holder /
                                       licensing body, never this
                                       actor. This actor coordinates
                                       DUPLICATION of already-licensed
                                       content only; it never grants,
                                       verifies, or self-issues the
                                       license that makes reproduction
   ...
   10. Invalid content-type        -- for `:log-production-batch`, if
                                       ...
                                       let a fabricated content type
                                       through.
   11. Invalid disc-thickness-mm   -- for `:log-production-batch`, if
                                       the patch declares a
                                       `:disc-thickness-mm` that is
                                       not a physically plausible
                                       reproduced-disc thickness
                                       reading
                                       (`mediarepro.registry/
                                       disc-thickness-mm-valid?`), the
   ...
   13. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [mediarepro.registry :as registry]
...
(def allowed-proposal-effects
  ...
  -- all four are propose-shaped drafts, NEVER a direct duplication-
  line-equipment-control effect."
...
(defn- equipment-control-blocked-violations
  ...
  Anything else (direct duplication-line-equipment control, a
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :equipment-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") は複製ライン設備の直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- equipment-actuate-blocked-violations
  ...
  to directly actuate duplication-line equipment -- this actor may
  only ever propose/schedule a DRAFT maintenance window, never actuate
  the equipment directly. No override, ever."
  [{:keys [op]} proposal]
  (when (and (= op :schedule-maintenance)
             (true? (:actuate-equipment? (:value proposal))))
    [{:rule :equipment-actuate-blocked
      :detail "複製ライン等の設備の直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))

(defn- copyright-license-authority-blocked-violations
  "HARD, PERMANENT, unconditional: ANY proposal (any op) whose own
  `:value`/`:patch` declares `:issue-reproduction-license? true` is
  attempting to self-issue a copyright reproduction-license/clearance
  authorization -- an authority exclusively reserved to the rights
  holder/licensing body, never this actor. This actor coordinates
  DUPLICATION of already-licensed content only. No phase and no human
  approval can ever override this."
  [proposal]
  (let [payload (or (:value proposal) (:patch proposal))]
    (when (true? (:issue-reproduction-license? payload))
      [{:rule :copyright-license-authority-blocked
        :detail "著作権複製許諾/ライセンスの自己発行提案は恒久的に禁止 -- 権利者・ライセンス団体の専権事項"}])))
...
(defn- invalid-product-type-violations
  ...
  let a fabricated content type through."
  [{:keys [op]} proposal]
...
(defn- invalid-disc-thickness-mm-violations
  "For `:log-production-batch`, if the patch declares a
  `:disc-thickness-mm` that is not a physically plausible reproduced-
  disc thickness reading, reject rather than let fabricated/sensor-
  error data through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [mm (:disc-thickness-mm (:value proposal))]
      (when (and (some? mm) (not (registry/disc-thickness-mm-valid? mm)))
        [{:rule :invalid-disc-thickness-mm
          :detail (str mm "mm は物理的に妥当な複製ディスク厚の範囲外")}]))))
...
(defn check
  "Censors a MediaReproductionAdvisor proposal against the governor
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (copyright-license-authority-blocked-violations proposal)
                           ...
                           (invalid-disc-thickness-mm-violations request proposal)
