(ns mediarepro.operation
  "MediaReproductionOperationActor -- one plant-operations coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (MediaReproductionAdvisor) is sealed into a
  single node (:advise); its proposal is ALWAYS routed through the
  Media Reproduction Plant Operations Governor (:govern) and the
...
(defn build
  "Compiles a MediaReproductionOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `mediarepro.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; MediaReproductionAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Media Reproduction Plant Operations Governor -- independent censor.
      (g/add-node :govern
