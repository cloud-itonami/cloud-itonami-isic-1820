(ns mediarepro.sim
  ...
  production quantity, a proposal that tries to ACTUATE duplication-
  line equipment directly (permanently blocked, no override), a
  proposal that tries to self-issue a copyright reproduction-license/
  clearance authorization (permanently blocked, no override), a
  double-schedule of the same maintenance window, a production-batch
  patch with a fabricated content type, a production-batch patch with
  an implausible disc-thickness-mm reading, and a production-batch
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on repl-001 (verified, registered, disc-replication line -- escalates, approve) ==")
    ...
    (println "== flag-safety-concern concern-1 on repl-001 (always escalates -- approve) ==")
    ...
    (println (exec-op actor "t6"
                       {:op :actuate-duplication-line :effect :propose :subject "batch-001"}
                       coordinator))

    (println "== schedule-maintenance mnt-2 on dup-002 (UNVERIFIED/unregistered duplication line -> HARD hold) ==")
    ...
    (println "== coordinate-shipment ship-3 on batch-002 (300 units would exceed quantity 2000 vs shipped 1800 -> HARD hold) ==")
    ...
    (println "== schedule-maintenance mnt-3 on repl-001 with :actuate-equipment? true -> HARD hold, PERMANENT, never reaches a human ==")
    ...
    (println "== log-production-batch batch-001 with a fabricated content-type -> HARD hold ==")
    (println (exec-op actor "t12"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:product-type :unobtainium}}
                       coordinator))

    (println "== log-production-batch batch-001 with an implausible disc-thickness-mm -> HARD hold ==")
    (println (exec-op actor "t13"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:disc-thickness-mm 999999.0}}
                       coordinator))
    ...
    (println "== log-production-batch batch-001 attempting to self-issue a copyright reproduction license -> HARD hold, PERMANENT ==")
