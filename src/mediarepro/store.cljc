(ns mediarepro.store
  "SSoT for the media-reproduction plant-operations coordination
  actor, behind a `Store` protocol so the backend is a swap, not a
  rewrite -- the same seam every `cloud-itonami-isic-*` actor in this
  fleet uses.

  Scope note: like its siblings (`cloud-itonami-isic-2680`'s own
  `magopticalmedia.store`, `cloud-itonami-isic-2710`'s own
  ...
  is self-contained (no external media-reproduction capability
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             content-type/disc-thickness-mm/quantity/
                             defect-rate record. `:verified?` marks
                             whether the batch's own claims have
    ...
    - `equipment`         -- a duplication-line unit's own record
                             (disc-replication line, software-
                             duplication line, or print-and-package
                             line). `:verified?`/`:registered?` track
                             whether it has actually been inspected/
                             commissioned and is on file -- the same
                             ground-truth discipline as `batches`.
    - `maintenance`       -- a scheduled duplication-line maintenance-
                             window DRAFT against a piece of equipment
                             (`mediarepro.registry`'s
    ...
    - `shipments`         -- a proposed outbound reproduced-media
                             product shipment DRAFT
                             (`mediarepro.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (mediarepro.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (mediarepro.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-type :cd
                :model "CD-Audio-Repro-Run" :disc-thickness-mm 1.2
                :quantity-units 5000.0 :defect-rate-percent 0.8
                :verified? true :registered? true
                :shipped-units 1000.0
                :last-assessed "2026-07-01"}
   "batch-002" {:id "batch-002" :product-type :dvd
                :model "DVD-Video-Repro-Run" :disc-thickness-mm 1.2
                :quantity-units 2000.0 :defect-rate-percent 0.5
                :verified? true :registered? true
                :shipped-units 1800.0
                :last-assessed "2026-07-01"}
   "batch-003" {:id "batch-003" :product-type :software-media
                :model "Software-Disc-Repro-Run" :disc-thickness-mm 1.2
                :quantity-units 3000.0 :defect-rate-percent 1.5
                :verified? false :registered? false
                :shipped-units 0.0
                :last-assessed "2026-06-15"}})

(defn- sample-equipment []
  {"repl-001" {:id "repl-001" :kind :disc-replication-line
               :verified? true :registered? true
               :last-maintenance-date "2026-06-01"}
   "dup-002" {:id "dup-002" :kind :software-duplication-line
              :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `mediarepro.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `mediarepro.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  disc-replication-line unit (schedulable for maintenance), one
  UNVERIFIED/unregistered software-duplication-line unit (blocks any
  maintenance scheduling against it) -- so the actor + demo + tests run
