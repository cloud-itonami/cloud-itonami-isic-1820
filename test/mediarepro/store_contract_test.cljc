(ns mediarepro.store-contract-test
  "The Store contract as executable tests. Single MemStore backend --
  see `mediarepro.store` ns docstring for why a second (Datomic-
...
(deftest sample-data-read-basics
  (let [s (seeded)]
    ...
    (is (true? (:verified? (store/equipment-unit s "repl-001"))))
    (is (true? (:registered? (store/equipment-unit s "repl-001"))))
    (is (false? (:verified? (store/equipment-unit s "dup-002"))))
    (is (false? (:registered? (store/equipment-unit s "dup-002"))))
    (is (= ["dup-002" "repl-001"] (mapv :id (store/all-equipment s))))
    (is (= [] (store/ledger s)))
...
(deftest maintenance-schedule-commits-and-advances-sequence
  (testing "commit-record! (like every sibling actor's own MemStore) returns the store `s`, not the domain result -- inspect the store directly, matching the discipline the actor's own :commit node relies on"
    (let [s (seeded)]
      ...
      (is (= "repl-001" (:equipment-id (store/maintenance s "mnt-1"))))
...
(deftest safety-concern-flag-appends
  (let [s (seeded)]
    ...
    (store/commit-record! s {:effect :safety-concern/flag :path ["concern-2"]
                             :value {:equipment-id "dup-002" :severity :high}})
...
(deftest shipment-propose-commits-and-advances-sequence-and-batch-quantity
  (let [s (seeded)]
    ...
    (is (= 1500.0 (:shipped-units (store/batch s "batch-001")))
        "1000.0 seeded + 500.0 committed")))
