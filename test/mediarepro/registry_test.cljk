(ns mediarepro.registry-test
  (:require [clojure.test :refer [deftest is]]
            [mediarepro.registry :as r]))
...
(deftest shipment-that-pushes-past-quantity-exceeds
  (is (true? (r/shipment-quantity-exceeded?
              {:quantity-units 2000.0 :shipped-units 1800.0} 300.0))))

(deftest shipment-exactly-at-quantity-does-not-exceed
  (is (false? (r/shipment-quantity-exceeded?
               {:quantity-units 2000.0 :shipped-units 1800.0} 200.0))
...
(deftest known-product-types-are-valid
  (doseq [pt [:cd :dvd :blu-ray
              :software-media :magnetic-tape]]
...
;; ----------------------------- disc-thickness-mm-valid? -----------------------------

(deftest typical-disc-thickness-mm-is-valid
  (is (r/disc-thickness-mm-valid? 0.0))
  (is (r/disc-thickness-mm-valid? 0.04))
  (is (r/disc-thickness-mm-valid? 0.76))
  (is (r/disc-thickness-mm-valid? 1.2))
  (is (r/disc-thickness-mm-valid? 1.5)))

(deftest negative-disc-thickness-mm-is-invalid
  (is (not (r/disc-thickness-mm-valid? -1.0))))

(deftest excessive-disc-thickness-mm-is-invalid
  (is (not (r/disc-thickness-mm-valid? 999999.0)))
  (is (not (r/disc-thickness-mm-valid? 1.51))))

(deftest non-numeric-or-missing-disc-thickness-mm-is-invalid
  (is (not (r/disc-thickness-mm-valid? nil)))
  (is (not (r/disc-thickness-mm-valid? "1.2"))))
...
(deftest history-is-append-only
  (let [c1 (r/register-maintenance "mnt-1" "repl-001" 0)
        hist (r/append [] c1)
        c2 (r/register-maintenance "mnt-2" "repl-001" 1)
