(ns mediarepro.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control duplication-line equipment
  directly... does NOT authorize or execute maintenance/equipment
  operations... does NOT self-issue a copyright reproduction-license/
  clearance authorization') implemented faithfully. The single
  invariant under test:

    MediaReproductionAdvisor never schedules maintenance, flags a
    safety concern, or coordinates a shipment the Media Reproduction
    Plant Operations Governor would reject;
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-type :cd}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :cd (:product-type (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-duplication-line :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
...
(deftest copyright-license-authority-is-held-and-permanently-blocked
  (testing "a proposal that sets :issue-reproduction-license? true -> HOLD, PERMANENT, never reaches request-approval -- this actor is never the copyright reproduction-license/clearance authority"
    (let [[db actor] (fresh)
          res (exec-op actor "t8b"
                    {:op :log-production-batch :effect :propose :subject "batch-001"
                     :patch {:issue-reproduction-license? true}}
                    coordinator)]
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:copyright-license-authority-blocked} (-> (store/ledger db) last :basis)))
      (is (not (true? (:issue-reproduction-license? (store/batch db "batch-001"))))
          "fabricated self-issued license never lands in the SSoT"))))
...
(deftest invalid-disc-thickness-mm-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10b" {:op :log-production-batch :effect :propose :subject "batch-001"
                                   :patch {:disc-thickness-mm 999999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-disc-thickness-mm} (-> (store/ledger db) last :basis)))
    (is (not= 999999.0 (:disc-thickness-mm (store/batch db "batch-001"))) "fabricated disc-thickness-mm never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "repl-001" :severity :moderate
                                            :description "stamper mechanism vibration anomaly"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "repl-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t13")]
