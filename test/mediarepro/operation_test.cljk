(ns mediarepro.operation-test
  "Smoke tests for the compiled MediaReproductionOperationActor graph
  itself (build + one happy path per op). The governor's full rule
  contract (HARD holds, escalation, phase gating) is exercised in
  `mediarepro.governor-contract-test`; the Store contract in
  `mediarepro.store-contract-test`."
...
(deftest test-actor-builds
  (testing "MediaReproductionOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "repl-001" :severity :moderate :description "stamper mechanism vibration anomaly"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
